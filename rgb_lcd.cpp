...
rgb_lcd::rgb_lcd()
{
    I2C_LCD = new Ci2c('1', LCD_ADDRESS); // new objet de la classe Ci2c
    I2C_RGB = new Ci2c('1', RGB_ADDRESS);
...
void rgb_lcd::i2c_send_byte(unsigned char dta)
{
    ...
    buffer[0] = dta;
    I2C_RGB->writeToI2C(buffer, 2);
    usleep(200);
}

void rgb_lcd::i2c_send_byteS(unsigned char *dta, unsigned char len)
{
	I2C_LCD->writeToI2C(dta, len);
}



void rgb_lcd::begin(int cols, int lines, int dotsize) 
{
...
void rgb_lcd::setCursor(int col, int row)
{
...
void rgb_lcd::createChar(int location, int charmap[])
{
...
// send command
inline void rgb_lcd::command(int value)
{
...
// send data
int rgb_lcd::write(string text)
{
    for(int i=0;i<text.length();i++)
    {
       unsigned char dta[2] = {0x40, text[i]};
       i2c_send_byteS(dta, 2);
    }
